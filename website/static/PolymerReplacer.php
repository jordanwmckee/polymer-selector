...
<body>
    ...
    <!-- end header section -->
     ...
     <div id="page-title" style="padding: 50px 0; width: 100%; background-color: #E2d0f3; text-align: center; color: white;">
        <h1>Polyer Replacement Tool</h1>
        <br>
        <a href="SearchPolyReplace.php">New Search</a>
    </div>
    ...
    while ($j < count($qtable)) {
        if ($j == 0) {
            if (count($qtable) == 1) {
                ...
                $joinner = $joinner . "`" . $qtable[$j] . "` JOIN";
                $onlinker = $onlinker . "`" . $qtable[$j] . "`.`polymer`=";
            }
        } else {
            if ($j == count($qtable) - 1) {
                $joinner = $joinner . "`" . $qtable[$j] . "`";
                $onlinker = $onlinker . "`" . $qtable[$j] . "`.`polymer`";
    ...
    for ($z = 0; $z < count($qtable); $z++) {
        $outerarray = array();
        $above = $row[$attribute_list[$z]] * 1.1;
        $below = $row[$attribute_list[$z]] * .9;
